[![MasterHead](https://media0.giphy.com/media/Y1vohJMVMtjSQxmUot/source.gif)](https://github.com/Waards)

<div align="center">

# Hi 👋, I'm Waards (Ken)

### Full-Stack Developer | API Integration Specialist | Automation Enthusiast

#### 🇵🇭 Based in the Philippines

</div>

<img align="right" alt="coding" width="400" src="https://c.tenor.com/GfSX-u7VGM4AAAAC/coding.gif">

## 👨‍💻 About Me

- 🎓 Currently studying at **CvSU**
- 💼 Building **CIANAN** - A full-stack multi-role web application (Stock Management + HR System)
- 🌱 Constantly learning and expanding my tech stack
- 🎨 Love creating playful, interactive UI elements and animations
- 🤖 Passionate about AI integration and workflow automation
- 📫 Reach me at **Wardswannabe@gmail.com**
- ⚡ Fun fact: **I combine serious development with creative flair - my dashboards have desk buddy pets!**

<br clear="right"/>

## 🚀 Current Projects

### CIANAN Web Application
A comprehensive multi-role system featuring:
- **Stock Management Module** - Inventory tracking, transaction management, print receipts
- **HR Module** - Recruitment pipeline, attendance monitoring, employee management
- **Multi-Dashboard System** - Super Admin, Admin, and HR role-based interfaces
- **Tech Stack**: PHP/PDO, MySQL, Vanilla JS, Chart.js, Session-based auth

**Special Features:**
- 🌸 Animated sakura petal effects with 3D CSS
- 🎮 Interactive desk buddy widgets (Pokémon, Doraemon, BTS chibis)
- 📊 Weighted attendance percentage system
- 📄 Excel export per employee, print reports

---

## 🛠️ Tech Stack

### Frontend Frameworks & Libraries
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,vue,typescript,nextjs,nuxtjs,tailwind,sass&theme=dark" alt="Frontend Skills" />
  </a>
</p>

**Specialized Skills:**
- GSAP animations
- Locomotive Scroll
- Pixel art & interactive UI widgets

### Backend & Frameworks
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=php,laravel,nodejs,mysql&theme=dark" alt="Backend Skills" />
  </a>
</p>

- **Laravel** - MVC architecture, Eloquent ORM
- **Node.js** - Backend services, API development
- **PHP/PDO** - Database interactions, session management
- **RESTful API Design** - Clean, scalable endpoints

### AI, Automation & API Integration

<table align="left">
  <tr>
    <td align="center">
      <a href="https://openrouter.ai/" target="_blank" rel="noreferrer">
        <img src="https://avatars.githubusercontent.com/u/126733545?s=200&v=4" alt="OpenRouter" width="50" height="50" />
      </a>
    </td>
    <td align="center">
      <a href="https://groq.com/" target="_blank" rel="noreferrer">
        <img src="https://avatars.githubusercontent.com/u/74888687?s=200&v=4" alt="Groq" width="50" height="50" />
      </a>
    </td>
    <td align="center">
      <a href="https://n8n.io/" target="_blank" rel="noreferrer">
        <img src="https://n8n.io/favicon.ico" alt="n8n" width="50" height="50" />
      </a>
    </td>
    <td align="center">
      <a href="https://www.gohighlevel.com/" target="_blank" rel="noreferrer">
        <img src="https://cdn.brandfetch.io/idK-hvK7Lv/w/200/h/200/theme/dark/icon.jpeg?c=1bxid64Mup7aczewSAYMX&t=1692001846848" alt="GoHighLevel" width="50" height="50" />
      </a>
    </td>
  </tr>
</table>

<br clear="left"/>

**Current API & Automation Arsenal:**
- 🤖 **OpenRouter API** - Multi-model AI routing
- 💻 **OpenCode API** - AI-powered code assistance
- ⚡ **Groq API** - Ultra-fast LLM inference
- 🔧 **n8n** - Open-source workflow automation with 400+ integrations
- 📈 **GoHighLevel (GHL)** - All-in-one CRM & marketing automation platform
- 🔌 **RESTful APIs** - Custom backend integrations

**Automation Focus:**
- Workflow automation with n8n
- AI-assisted development
- CRM automation (GoHighLevel)
- Intelligent data processing

### Core Languages
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=javascript,typescript,html,css&theme=dark" alt="Core Languages" />
  </a>
</p>

### Tools & DevOps
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,firebase,photoshop,unity&theme=dark" alt="Tools & DevOps" />
  </a>
</p>

---

## 💪 Core Competencies (Summary)

- **Frontend:** React, Vue, TypeScript, Next.js, Nuxt, Tailwind, GSAP  
- **Backend:** PHP (Laravel, PDO), Node.js, MySQL, REST APIs  
- **AI & Automation:** OpenRouter, Groq, n8n, GoHighLevel, workflow automation  
- **Specialties:** Multi‑role systems, interactive UI, creative development  

> Full‑stack developer focused on clean code, smooth UX, and AI‑powered workflows.

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Waards&show_icons=true&theme=tokyonight" alt="GitHub Stats" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Waards&theme=tokyonight" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Waards&layout=compact&theme=tokyonight" alt="Top Languages" />
</p>

---

## 🌐 Connect With Me

<table align="left">
  <tr>
    <td align="center">
      <a href="https://twitter.com/wardseverything" target="_blank">
        <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/twitter.svg" alt="Twitter" width="40" height="40" />
      </a>
    </td>
    <td align="center">
      <a href="https://discord.gg/RYhYaxjE" target="_blank">
        <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/discord.svg" alt="Discord" width="40" height="40" />
      </a>
    </td>
    <td align="center">
      <a href="https://github.com/Waards" target="_blank">
        <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/github.svg" alt="GitHub" width="40" height="40" />
      </a>
    </td>
  </tr>
</table>

<br clear="left"/>

---

## 🏆 Featured Work

### 🎨 Creative UI Components
- Sakura petal falling animations (3D CSS effects)
- Interactive desk buddy widgets with WASD controls
- Character selection systems (Pokémon, Doraemon, BTS)
- Behavioral state machines for UI pets

### 🔧 System Development
- Multi-role authentication system
- Weighted attendance calculation algorithms
- Excel export functionality with formatting
- Print report generation
- Soft delete architecture

### 🤖 AI & Automation
- OpenRouter API integration for multi-model inference
- Groq API implementation for high-speed LLM responses
- Custom REST APIs for system integration
- Workflow automation with n8n
- CRM automation pipelines with GoHighLevel

### 📁 Top Repositories (by activity)

| Repository | Description |
|------------|-------------|
| [**Waards**](https://github.com/Waards/Waards) | Profile README repo |
| [**gittestair**](https://github.com/Waards/gittestair) | Testing deployment playground |
| [**Hotel Management Software**](https://github.com/Waards/Hotel-Management-Software) | Hotel management system in PHP |
| [**ECHO-LOGIN**](https://github.com/Waards/ECHO-LOGIN) | Authentication system |
| [**bot**](https://github.com/Waards/bot) | Automation bot projects |
| [**works**](https://github.com/Waards/works) | Work-related experiments |
| [**connect**](https://github.com/Waards/connect) | TypeScript connectivity tools |
| [**craft**](https://github.com/Waards/craft) | TypeScript creative coding |
| [**vas**](https://github.com/Waards/vas) | TypeScript utilities |
| [**bolt.diy**](https://github.com/Waards/bolt.diy) | Fork of stackblitz-labs/bolt.diy |

---

## 📫 Let's Build Something Amazing

I'm always open to:
- 💼 Collaboration opportunities
- 🚀 Open source contributions
- 💡 Innovative project ideas
- 🤝 Networking with fellow developers

**Reach out:** Wardswannabe@gmail.com

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Waards&label=Profile%20Views&color=0e75b6&style=flat" alt="Profile Views" />
</p>

<p align="center">
  <i>⭐ From <a href="https://github.com/Waards">Waards</a> - Building the future, one commit at a time</i>
</p>
